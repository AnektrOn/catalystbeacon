# 📧 Function Node - Template Email Nouvel Utilisateur

## 🎯 Code Complet pour Function Node N8N
...
### Option 1 : Function Node + SMTP Node

1. **Function Node** : Utilisez le code ci-dessus (génère `to`, `subject`, `html`)
2. **SMTP Node** : Configurez avec :
   - **To Email** : `{{ $json.to }}`
...
Si vous préférez mettre le HTML directement dans le SMTP Node, utilisez le template de `N8N_TEMPLATE_NEW_USER.md` avec les variables N8N.

---

## 🔧 Variables Disponibles

- `emailData.email` - Email de l'utilisateur
- `emailData.userName` - Nom complet (ou 'there' si vide)
- `siteUrl` - URL du site (depuis `$env.SITE_URL` ou défaut)
- `siteName` - Nom du site (depuis `$env.SITE_NAME` ou défaut)
...
- [ ] SMTP Node configuré pour utiliser les sorties du Function Node
