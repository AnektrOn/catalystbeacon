-- ============================================
-- Fix next_lesson_id in complete_lesson_transaction
-- ============================================
-- 1) Initialize v_next_lesson_id to NULL explicitly.
-- 2) Get next node by taking the first row from generate_roadmap_nodes that
--    is NOT the lesson we just completed (so we never return the same lesson).
...
BEGIN
  ...
  -- Next node: first row from generate_roadmap_nodes that is NOT the lesson we just completed
  SELECT l.lesson_id INTO v_next_lesson_id
  FROM generate_roadmap_nodes(p_user_id, p_masterschool, 5) l
  WHERE l.lesson_id IS DISTINCT FROM p_lesson_id
  LIMIT 1;
...
COMMENT ON FUNCTION complete_lesson_transaction(UUID, TEXT, INT, INT, INT, TEXT, TEXT) IS
  'Completes a lesson, awards XP, records lesson_completion_events, and returns next_lesson_id (skips current lesson).';
